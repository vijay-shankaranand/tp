...
7. **Enforce stricter deadline date for `add_task` command**
   ...
   As an example, for `add_event`, there will be another parameter say `days_past_event` and let the users input the number of days past the event deadline for which tasks can be added.
